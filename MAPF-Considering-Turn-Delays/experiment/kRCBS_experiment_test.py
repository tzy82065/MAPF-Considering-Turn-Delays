...
class Node:
    def __init__(self, node_id, current_cost, constraints, plan_x, plan_y):
    ...
    def __repr__(self):
        return (f"Node({self.node_id}, Cost: {self.current_cost}, Constraints: {self.constraints}, "
    ...
    def print_tree(self, level=0):
        indent = " " * (level * 4)  # 按生成层数缩进
        print(
            f"{indent}Node({self.node_id}, Cost: {self.current_cost}, Constraints: {self.constraints}, "
            f"plan_x: {self.plan_x}, plan_y: {self.plan_y})")
        for child in self.children:
            child.print_tree(level + 1)  # 递归调用打印子节点


def update_cost(parent_cost):
    return parent_cost + 1


def update_constraints_left(conflict, parent_constraints):
    new_constraints = (conflict[0], conflict[2], conflict[3], conflict[-1])
    parent_constraints.append(new_constraints)
    constraints = parent_constraints
    return constraints


def update_constraints_right(conflict, parent_constraints):
    new_constraints = (conflict[1], conflict[2], conflict[3], conflict[-1])
    parent_constraints.append(new_constraints)
    constraints = parent_constraints
    return constraints


def insert_pos(rx, ry, t):
    if not rx or not ry or len(rx) != len(ry):  # 确保两个列表都不为空且长度相等
        return rx, ry
    k = t
    while k > 0 and rx[k - 1] == rx[t] and ry[k - 1] == ry[t]:
        k -= 1
    # 现在 i 指向最后一个非重复片段的元素
    if k > 0:
        rx.insert(k, rx[k - 1])
...
def update_plan(conflict, parent_x_plan, parent_y_plan):
    if conflict is not None:
        conflict_time = conflict[-1]
        if 0 <= conflict_time < len(parent_x_plan):
            updated_x_plan, updated_y_plan = insert_pos(parent_x_plan, parent_y_plan, conflict_time)
        else:
            print('Out of plan')
    return updated_x_plan, updated_y_plan


# 检查是否存在Deadlock（一条路径完全包含于另一条路径）
def check_subsequence(list_1, list_2):
    for start_idx in range(len(list_2) - len(list_1) + 1):
        if list_1 == list_2[start_idx:start_idx + len(list_1)]:
    ...
    for i in range(n):
        for j in range(n):
            if i != j and len(rx[i]) <= len(rx[j]) and len(ry[i]) <= len(ry[j]):
                ...
                for l in range(len(rx[i])):
                    for k in range(len(rx[j])):
                        if ((rx[i][0] == rx[j][k] and ry[i][0] == ry[j][k] and
                             rx[i][l] == rx[j][0] and ry[i][l] == ry[j][0]) or
                                (rx[i][-1] == rx[j][k] and ry[i][-1] == ry[j][k] and
                                 rx[i][l] == rx[j][-1] and ry[i][l] == ry[j][-1])):
                            result.append((i, j))
    return result


# 检查是否存在k顶点冲突
def find_k_vertex_conflict(node, k):
    ...
    # 如果在k个时间步下坐标相同，则返回冲突的时间和位置
    for i in range(len(node.plan_x)):
        # 遍历当前子列表之后的子列表以避免重复比较
        for j in range(len(node.plan_x)):
            if i != j:
                # 遍历子列表rx[i]中的每个元素
                # 检查是否存在k延迟冲突
                for t in range(len(node.plan_x[i])):
                    for delay in range(k + 1):
                        if t + delay < max_length:
                            if (node.plan_x[i][t] == node.plan_x[j][t + delay] and
                                    node.plan_y[i][t] == node.plan_y[j][t + delay]):
                                # 如果存在冲突，返回相关信息
                                return (i, j, node.plan_x[i][t], node.plan_y[i][t],
                                        node.plan_x[j][t + delay], node.plan_y[j][t + delay], t + delay, t)

    return None


# 检查是否存在交换/合并冲突
def find_k_swapping_conflict(node, k):
    overlap_subseq = None
    max_length = 0
        ...
        for j in range(i + 1, list_length):
            ...
            for length in range(min_sub_length, 1, -1):
                for t in range(min_sub_length - length + 1):
                    ...
                    # 检查是否是倒序切片，并确保前后k个索引内没有相同的或倒序的切片
                    if sub_rx_i == sub_rx_j[::-1] and sub_ry_i == sub_ry_j[::-1] and length > max_length:
                        # 确保前后k个索引内没有相同的或倒序的切片
                        conflict = False
                        # 检查前k个索引内的切片
                        for delay in range(1, k + 1):
                            if t - delay >= 0:
                                if (node.plan_x[i][t - delay:t - delay + length] == node.plan_x[j][
                                                                                    t - delay:t - delay + length] or
                                        node.plan_x[i][t - delay:t - delay + length] == node.plan_x[j][
                                                                                        t - delay:t - delay + length][
                                                                                        ::-1]):
                                    conflict = True
                                    break
                            if t + length + delay <= min_sub_length:
                                if (node.plan_x[i][t + length:t + length + delay] == node.plan_x[j][
                                                                                     t + length:t + length + delay] or
                                        node.plan_x[i][t + length:t + length + delay] == node.plan_x[j][
                                                                                         t + length:t + length + delay][
                                                                                         ::-1]):
                                    conflict = True
                                    break
                        ...
                        if not conflict:
                            ...
                            overlap_subseq = {
                                'i': i, 'j': j,
                                'time': t, 'length': length, 'time+delay': t + delay,
    ...
    if overlap_subseq is not None:
        return (overlap_subseq['i'], overlap_subseq['j'], overlap_subseq['sub_rx_i'], overlap_subseq['sub_ry_i'],
                overlap_subseq['length'], overlap_subseq['time+delay'], overlap_subseq['time'])
    else:
        return None


# 检查是否存在冲突，如有返回冲突信息元组，如无返回None
def check_goal(k_conflict, s_conflict):
    if s_conflict is not None:
        conflict = s_conflict
    elif k_conflict is not None:
        conflict = k_conflict
    else:
        conflict = None
    return conflict
...
def generate_cbs_tree(root):
    queue = [root]
    while queue:
        current_node = queue.pop(0)
        # 检查当前节点是否满足条件
        conflict = check_goal(find_k_vertex_conflict(current_node, 1), find_k_swapping_conflict(current_node, 1))
        # 满足条件后结束生成
        if conflict is None:
            break
        i_left = conflict[0]
        j_right = conflict[1]
        # 更新子节点属性并添加到树([:]用于创建被修改列表的副本以避免原地修改)
        # 根据冲突分别添加约束
        left_constraints = update_constraints_left(conflict, current_node.constraints[:])
        right_constraints = update_constraints_right(conflict, current_node.constraints[:])
        # 更新路径方案
        # 修改左节点路径方案
        left_plan_x = copy.deepcopy(current_node.plan_x)
        left_plan_y = copy.deepcopy(current_node.plan_y)
        left_plan_x[i_left], left_plan_y[i_left] = update_plan(conflict, left_plan_x[i_left], left_plan_y[i_left])
        left_child = Node(current_node.node_id * 2, current_node.current_cost + 1, left_constraints,
                          left_plan_x,
                          left_plan_y)
        # 修改右节点路径方案
        right_plan_x = copy.deepcopy(current_node.plan_x)
        right_plan_y = copy.deepcopy(current_node.plan_y)
        right_plan_x[j_right], right_plan_y[j_right] = update_plan(conflict, right_plan_x[j_right],
                                                                   right_plan_y[j_right])
        right_child = Node(current_node.node_id * 2 + 1, current_node.current_cost + 1, right_constraints,
                           right_plan_x,
                           right_plan_y)
...
# 将字符串表示的列表转换为整数列表的函数
def convert_to_int_list(list_str):
    # 使用ast.literal_eval安全地将字符串解析为列表
    string_list = ast.literal_eval(list_str)
    # 将列表中的每个元素转换为整数
...
def main():
    ...
    for test_number in range(1, 51):
        ...
        sx, sy, gx, gy = generate_points_for_special_map(size_x_min=-1, size_x_max=31, size_y_min=4, size_y_max=34,
                                                         ox=ox,
                                                         oy=oy, num_points=num_points, min_distance=1)
        '''
        sx, sy, gx, gy = generate_points(size_x=161, size_y=61, ox=ox, oy=oy, num_points=num_points, min_distance=1)
        # 网格大小
        grid_size = 1.0
        # 智能体半径
        agent_radius = 0.5
        ...
        rx, ry = [], []
        '''
        sx, sy, gx, gy = genera te_points_for_special_map(size_x_min=-1, size_x_max=31, size_y_min=4, size_y_max=34,
                                                         ox=ox,
        ...
        if len(deadlock) != 0:
            ...
            result = run_with_timeout(generate_cbs_tree, args=(root_node,), timeout_duration=300)
            if result is None:
    ...
    df.to_excel(f'{num_points}_3.xlsx', index=False)
    print(f"----------k={num_points}结果写入完成----------")
    print(f"----------成功率为{success_result*2}%----------")
