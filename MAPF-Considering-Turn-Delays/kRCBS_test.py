...
def update_plan_left(conflict, parent_x_plan, parent_y_plan):
...
def update_plan_right(conflict, parent_x_plan, parent_y_plan):
    if conflict is not None:
        conflict_time = conflict[-2]
        if 0 <= conflict_time < len(parent_x_plan):
            updated_x_plan, updated_y_plan = insert_pos(parent_x_plan, parent_y_plan, conflict_time)
        else:
            print('Out of plan')
...
def generate_cbs_tree(root):
    ...
    while queue:
        ...
        # 检查当前节点是否满足条件
        conflict = check_goal(find_k_vertex_conflict(current_node, k=1), find_k_swapping_conflict(current_node, k=1))
        # 满足条件后结束生成
        if conflict is None:
            break
        ...
        left_plan_x[i_left], left_plan_y[i_left] = update_plan_left(conflict, left_plan_x[i_left], left_plan_y[i_left])
        ...
        right_plan_x[j_right], right_plan_y[j_right] = update_plan_right(conflict, right_plan_x[j_right],
...
def calculate_cost_makespan(plan_x, plan_y):
    makespan = len(plan_x[0])
    cost = 0
    rx, ry = simplify_ending(plan_x, plan_y)
    makespan = len(rx[0]) - 1
    for rxi in rx:
        if (len(rxi) - 1) > makespan:
            makespan = len(rxi) - 1
        cost += len(rxi) - 1
...
def main():
    ...
    # 非常规地图
    ox, oy = read_map_file('/Bachelorarbeit/map/random-32-32-10.map')
    ...
    sx, sy, gx, gy = generate_points_for_special_map(size_x_min=-1, size_x_max=31, size_y_min=4, size_y_max=34, ox=ox,
                                                     oy=oy, num_points=8, min_distance=1)
    sx = [18, 7, 21, 12]
    sy = [7, 30, 18, 17]
    gx = [24, 17, 9, 1]
    gy = [6, 32, 25, 16]
    ...
    # CBS，启动！
    result = run_with_timeout(generate_cbs_tree, args=(root_node,), timeout_duration=180)
    if result is None:
